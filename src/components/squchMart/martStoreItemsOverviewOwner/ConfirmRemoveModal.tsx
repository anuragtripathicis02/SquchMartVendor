...
const ConfirmRemoveModal = ({ show, handleClose, onConfirm }: any) => {
    ...
    const handleConfirm = () => {
        if (typeof onConfirm === 'function') {
            onConfirm(); // call only if function is passed
        }
        setConfirmed(true);     // flag to show success modal
        handleClose();          // close this modal
    };

    const handleExited = () => {
        if (confirmed) {
            setShowSuccess(true);  // show success modal only after exit
    ...
    return (
        <>
            <Modal
                ...
                centered
                className="host-profile-modal common-modal-sec modal-small-box pt-4"
            >
                ...
                <Modal.Body className="common-modal-body p-0 text-center">
                    <div className='modal-box-from p-4'>
                        ...
                        <h5 className="font-text">Are you sure you want to Delete this Promocode?</h5>
                    </div>
                </Modal.Body>
